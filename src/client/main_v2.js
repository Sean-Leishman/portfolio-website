...
jQuery(document).ready(function() {
  ...
  const hdrEquirect = new RGBELoader().load(HDR, async function(hdrmap) {
    ...
    await init(hdrmap);
    console.log("Initialise fullpage.js",models)
    await init_fp();
    console.log("Init Animatons",models)
    await animate();
    render();
  });
            ...
            switch (ball_merge_main_state) {
              ...
              case BALL_MERGE_MAIN_STATE.MERGE_IN_PROCESS:
                if (
                  ...
                  ball_merge_main_state =
                    BALL_MERGE_MAIN_STATE.MERGE_STAGE_DONE;
                } else {
                  merge();
                  animate_ball_motion(false);
                }
